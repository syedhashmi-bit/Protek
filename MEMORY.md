...
The running host is `<live-hostname>`, Hetzner **Helsinki 8GB**,
public IPv4 **`<live-host-ip>`** — a **snapshot/clone of VPS B**, not B itself
(`<vps-b-ip>`, Oregon). It inherited B's journal history (old `ubuntu-2gb-hil-1`
...
MT `<mikrotik-ip>:8728` only allowed B's IP; the clone's `<live-host-ip>` was blocked.
...
**Production host changed.** Protek now runs on **VPS B** (`<vps-b-ip>`, Ubuntu 26.04,
...
- Discovered `settings.dry_run='0'` had been flipped via /settings UI
  ...
  ~200 successful IPv4 adds per cycle to `<mikrotik-ip>` from at least
...
Live result against MikroTik at `<mikrotik-ip>`:
```json
...
- **MikroTik credentials** in `.env`: `MT_HOST=<mikrotik-ip>`, `MT_USERNAME=<mt-api-user>`, port 8728.
  RouterOS 7.22.1 on the home router ("<router-identity>"). Connection confirmed via `mikrotik.health()`.
...
- **Domain bound**: `protek.syedhashmi.trade` (DNS A → `<vps-a-ip>`, same as pipsqueeze)
